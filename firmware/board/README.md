...
The gift-fleet units run a custom-compiled MicroPython (pinned as the
...
| Minimal frozen manifest | `manifest.py` | Stock freezes bundle-networking (mip/ntptime/requests/webrepl) + aioble; the app uses none of them. App code is deliberately NOT frozen — frozen can't be OTA-updated, which is exactly what the ROMFS partition buys. |
...
changes — app code deploys via `python tools/build.py flash` over USB, or
over the air from the backend's `/app/*` endpoints
(`python tools/build.py publish-app --deploy`). The littlefs filesystem
(config.json, logs, app code) survives a firmware reflash.
