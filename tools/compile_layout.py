#!/usr/bin/env python3
"""Compile sprite/layout source art into framebuf modules, for both firmwares.

Reads source art from `firmware/assets/layout/` and writes:

  * generated `.py` modules to `firmware/src/scoreboard/layout/`, one per
    sprite or slice. The output directory is gitignored — these modules are
    build artifacts, not source code.
  * one generated `.rs` module to
    `crates/scoreboard-render/src/generated/layout.rs`, with a Rust module per
    sprite or slice. Unlike the MicroPython side this one is **committed**:
    regenerating it needs Pillow and the Aseprite CLI, and requiring either to
    build the firmware would put a GUI application in the cargo graph.

Both outputs come from ONE pass over the same packed bytes, so the two
firmwares cannot disagree about a pixel.

The `.aseprite` files are the source of truth.
...
RUST_OUTPUT = (
    REPO_ROOT / "crates" / "scoreboard-render" / "src" / "generated" / "layout.rs"
)
...
def rust_bytes_literal(data: bytes, per_line: int = 16, indent: str = "    ") -> str:
...
def rust_palette_literal(palette_bytes: bytes, per_line: int = 8, indent: str = "    ") -> str:
    """Emit the body of a Rust `[u16; N]` array literal from RGB565-LE bytes."""
    values = [
        int.from_bytes(palette_bytes[i : i + 2], "little")
...
def generate_rust_sprite(name: str, info: dict, source_desc: str, position=None) -> str:
    """One `pub mod <name>` holding a Sprite (and its POSITION when absolute)."""
    key = "None" if info["key"] == -1 else f"Some(0x{info['key']:04X})"
    lines = [
        f"/// `{source_desc}` — {info['mode_desc']}.",
        f"pub mod {name} {{",
        "    use super::*;",
        "",
        ...
        f"        width: {info['width']},",
        f"        height: {info['height']},",
        f"        format: PixelFormat::{_RUST_FORMATS[info['format_name']]},",
        f"        palette: {'Some(&PALETTE)' if info['palette'] is not None else 'None'},",
        f"        key: {key},",
        "    };",
    ]
    ...
    lines += [
        "",
        "    #[rustfmt::skip]",
        f"    static DATA: [u8; {len(info['data'])}] = [",
        rust_bytes_literal(info["data"], indent="        "),
        "    ];",
    ]
    if info["palette"] is not None:
        lines += [
            "",
            f"    /// RGB565, index 0 = transparent."
            if info["key"] != -1
            else "    /// RGB565.",
            "    #[rustfmt::skip]",
            f"    pub static PALETTE: [u16; {info['palette_count']}] = [",
            rust_palette_literal(info["palette"], indent="        "),
            "    ];",
        ]
    lines.append("}")
    return "\n".join(lines)
...
def generate_rust_slice(name: str, source_desc: str, x: int, y: int, w: int, h: int) -> str:
    return "\n".join(
        [
            f"/// `{source_desc}`.",
            f"pub mod {name} {{",
...
def compile_layout(png_path: Path, json_path: Path) -> list:
    ...
    Returns a list of (output_name, source_description, log_message,
    micropython_source, rust_source) tuples, one per emitted module. Layers
    without a `__relative` or `__absolute` suffix are silently skipped (logged
    as skipped in the caller if desired).
    """
    ...
    for layer_name, frame_entry in frames.items():
        ...
        if kind == "relative":
            ...
            rust_source = generate_rust_sprite(base_name, info, desc)
            log = (
                f"  layer {layer_name!r} -> {base_name} "
            ...
            rust_source = generate_rust_sprite(base_name, info, desc, position=(x, y))
            log = (
                f"  layer {layer_name!r} -> {base_name} "
                f"({info['mode_desc']}, absolute @ ({x}, {y}))"
            )

        outputs.append((base_name, desc, log, source, rust_source))
    ...
    for slice_entry in meta.get("slices", []):
        ...
        rust_source = generate_rust_slice(slice_name, desc, x, y, w, h)
        log = f"  slice {slice_name!r} -> {slice_name} ({w}x{h} @ ({x}, {y}))"

        outputs.append((slice_name, desc, log, source, rust_source))

    return outputs


def _register_output(outputs: dict, name: str, desc: str, source: str, rust_source: str) -> None:
    ...
    if name in outputs:
        ...
        sys.exit(1)
    outputs[name] = (desc, source, rust_source)
...
def compile_all() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if aseprite_files:
            ...
            for asp in aseprite_files:
                ...
                for out_name, desc, log_msg, source, rust_source in compile_layout(png, js):
                    _register_output(outputs, out_name, desc, source, rust_source)
                    print(log_msg)
        ...
        for png_path in plain_pngs:
            ...
            rust_source = generate_rust_sprite(stem, info, f"{stem}.png")
            _register_output(outputs, stem, f"{stem}.png", source, rust_source)
            print(f"  -> {stem} ({info['mode_desc']})")

    # Write new outputs, then remove stale modules that aren't in the new set.
    new_filenames = set()
    for out_name, (_desc, source, _rust) in sorted(outputs.items()):
    ...
    # The Rust side is one file, so it has no stale-module problem: a renamed
    # layer simply stops appearing in it.
    RUST_OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    rust_modules = [rust for _name, (_desc, _py, rust) in sorted(outputs.items())]
    ...
    rust_rel = RUST_OUTPUT.relative_to(REPO_ROOT).as_posix()
