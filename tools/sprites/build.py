...
1. Plain PNG (no matching .json sidecar):
   The entire PNG becomes one module. Each module exports `WIDTH`, `HEIGHT`,
   `data` (framebuf.FrameBuffer), and optionally `palette` (framebuf.FrameBuffer)
   when a paletted format is chosen.

2. PNG + matching .json sidecar (Aseprite sprite sheet export with Split Layers
   and JSON Data):
   The PNG is treated as an atlas of layers and the JSON is parsed for per-layer
   frames and per-slice coordinates. Layers are filtered by suffix:
...
def generate_module(
    ...
    *,
    ...
    """
    lines = [
        f"# Generated by tools/sprites/build.py from {source_desc}",
...
def main():
    ...
    # Collect everything first so collisions are detected before any file is written.
    outputs = {}  # stem -> (source_description, module_source_text)
    ...
    for png_path in pngs:
        ...
        if json_path.is_file():
            print(f"[{stem}] processing layout ({png_path.name} + {json_path.name})")
            compiled = compile_layout(png_path, json_path)
            for out_name, desc, log_msg, source in compiled:
    ...
    # Write all outputs now that we know there are no collisions.
    for out_name, (_desc, source) in sorted(outputs.items()):
        out_path = BUILD_DIR / f"{out_name}.py"
        out_path.write_text(source)

    print(f"\nDone: {len(outputs)} module(s) written to build/")
